...
const labelStyles = Object.assign({fontSize: 'calc(7px + .5vw)', position: 'absolute'}, noSelect)
...
class Chess extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    const hasSelected =
      selectedPiece && nextProps.pieces.some(piece => piece === selectedPiece.notation)
  ...
  handleSquareClick(evt) {
    const {onSelectPiece, onMovePiece} = this.props
    ...
    const currentlySelected = this.state.selectedPiece
    const clickedSquare = this.coordsToPosition({x: node.offsetLeft, y: node.offsetTop})
    ...
    const {x, y} = clickedSquare
    const selected = this.findPieceAtPosition(clickedSquare.pos)
    const selectedPiece = selected && Object.assign({x, y}, selected)
  ...
  handleDragStop(evt, drag) {
    ...
    const dragTo = this.coordsToPosition({x: node.offsetLeft + x, y: node.offsetTop + y})

    this.setState({dragFrom: null, targetTile: null, draggingPiece: null})

    const hasMoved = dragFrom.pos !== dragTo.pos
  ...
  render() {
    const {targetTile, selectedPiece, draggingPiece, boardSize} = this.state
    ...
    for (let y = 0; y < 8; y++) {
      for (let x = 0; x < 8; x++) {
        ...
        const boxShadow = isTarget || isSelected ? 'inset 0px 0px 0px 0.4vmin yellow' : undefined
        const styles = Object.assign({background, boxShadow}, squareStyles)

        tiles.push(
          <div key={`rect-${x}-${y}`} onClick={this.handleSquareClick} style={styles}>
